//Aldana del Viso
//tp3 - comision 3
//Video: https://youtu.be/d6pCaFnoRlk

PImage imagen;
int cantC, cantF, xdesp, tam, posX, posY;
float tamCirculo;
...
void draw(){
  ...
  for (int x =0; x < cantC; x++) {//columna
    for (int y =0 ; y < cantF; y++) {//fila
...
int calcularDesplazamiento(int y){
    int desp=0;
    int modY = y % 4;
    if (modY == 0 || modY==4) {
        desp = -30;
...
void cuadrado(int x, int y, int tam, int posActual){
  if(esPar(posActual)){
    if(estadoOriginal){
      fill(255);
    }else{
...
void circulo(int x, int y, float tam, int posActual){
   if(esPar(posActual)){
     fill(135, 206, 235);
   }else{
     fill(255);
